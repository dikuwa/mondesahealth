...
export async function PATCH(request: Request) {
  ...
  if (parsed.data.action === "READ_ALL") await db.notification.updateMany({ where: { userId: user.id, readAt: null }, data: { readAt: new Date() } });
  else if (parsed.data.id) await db.notification.updateMany({ where: { id: parsed.data.id, userId: user.id }, data: { readAt: new Date() } });
...
export async function DELETE(request: Request) {
  ...
  await db.notification.deleteMany({ where: { userId: user.id, ...(parsed.data.id ? { id: parsed.data.id } : {}) } });
  return NextResponse.json({ ok: true });
}
