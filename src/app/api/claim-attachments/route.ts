...
export async function POST(request: Request) {
  ...
  const target = parsed.data.claimId
    ? await db.claim.findFirst({ where: { id: parsed.data.claimId, practiceId: session.practiceId }, select: { id: true } })
    : parsed.data.batchId
      ? await db.claimBatch.findFirst({ where: { id: parsed.data.batchId, practiceId: session.practiceId }, select: { id: true } })
      : await db.patientMedicalAid.findFirst({ where: { id: parsed.data.patientMedicalAidId, practiceId: session.practiceId }, select: { id: true } });
  ...
  try{attachment=await db.$transaction(async tx=>{const[claimUsage,intakeUsage]=await Promise.all([tx.claimAttachment.aggregate({where:{practiceId:session.practiceId},_sum:{fileSize:true}}),tx.patientIntakeImage.aggregate({where:{intake:{practiceId:session.practiceId}},_sum:{fileSize:true}})]);if((claimUsage._sum.fileSize||0)+(intakeUsage._sum.fileSize||0)+file.size>limitMb*1024*1024)throw new Error("QUOTA");return tx.claimAttachment.create({ data: { ...parsed.data, practiceId:session.practiceId, filename: file.name, mimeType: file.type, fileSize: file.size, data, uploadedByUserId: session.id } })},{isolationLevel:Prisma.TransactionIsolationLevel.Serializable})}catch(error){if(error instanceof Error&&error.message==="QUOTA")return NextResponse.json({error:`Protected attachment storage limit (${limitMb} MB) would be exceeded.`},{status:413});if(error instanceof Prisma.PrismaClientKnownRequestError&&error.code==="P2034")return NextResponse.json({error:"Storage usage changed during upload. Try again."},{status:409});return NextResponse.json({error:"The attachment could not be stored."},{status:500})}
...
export async function GET(request: Request) {
  ...
  const attachment = await db.claimAttachment.findFirst({ where: { id, practiceId:session.practiceId } }); if (!attachment) return NextResponse.json({ error: "Attachment not found." }, { status: 404 });
