...
const saveSchema = z.object({ action: z.literal("SAVE"), id: z.string(), patientMedicalAidId: z.string().nullable().optional(), medicalAidFundId: z.string().nullable().optional(), claimType: z.enum(["DIRECT_MEDICAL_AID", "PATIENT_REIMBURSEMENT"]), serviceDateFrom: z.coerce.date(), serviceDateTo: z.coerce.date(), practitioner: z.string().trim().min(2), internalNotes: z.string().trim().max(2000).nullable().optional(), lines: z.array(line).min(1) });
...
export async function POST(request: Request) {
  ...
  const appointment = parsed.data.appointmentId ? await db.appointment.findFirst({ where: { id: parsed.data.appointmentId, practiceId:session.practiceId }, include: { patient: { include: { memberships: { where: { practiceId:session.practiceId,current: true }, include: { medicalAid: true } } } } } }) : null;
  ...
  const patient = appointment?.patient || await db.patient.findFirst({ where: { id: patientId,practiceId:session.practiceId }, include: { memberships: { where: { practiceId:session.practiceId,current: true }, include: { medicalAid: true } } } });
  if (!patient) return NextResponse.json({ error: "Patient not found." }, { status: 404 });
  const membership = patient.memberships[0], practice = await db.practiceSetting.findUnique({ where: { practiceId:session.practiceId } });
...
export async function PATCH(request: Request) {
  ...
  if (body.action === "SAVE") {
    ...
    const existing = await db.claim.findFirst({ where: { id: parsed.data.id,practiceId:session.practiceId } });
  ...
  if (body.action === "VALIDATE") {
    ...
    const id = z.string().parse(body.id);if(!await db.claim.findFirst({where:{id,practiceId:session.practiceId},select:{id:true}}))return NextResponse.json({error:"Claim not found."},{status:404});const result = await validateClaim(id);
  ...
  if (body.action === "OUTCOME") {
    ...
    const claim = await db.claim.findFirst({ where: { id: parsed.data.id,practiceId:session.practiceId } }); if (!claim) return NextResponse.json({ error: "Claim not found." }, { status: 404 });
  ...
  if (body.action === "RESUBMIT") {
    ...
    const original = await db.claim.findFirst({ where: { id: z.string().parse(body.id),practiceId:session.practiceId }, include: { lines: { include: { diagnosisCodes: true } } } });
