...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (note.status !== "ISSUED" || !note.verificationToken) return NextResponse.json({ error: note.status === "REVOKED" ? "This sick note has been revoked and cannot be downloaded." : "Only issued sick notes can be downloaded." }, { status: 409 });
  const origin = process.env.NEXT_PUBLIC_SITE_URL?.replace(/\/$/, "") || new URL(request.url).origin;
  const verificationUrl = `${origin}/verify/sick-note/${encodeURIComponent(note.verificationToken)}`;
  const qrDataUrl = await QRCode.toDataURL(verificationUrl, { errorCorrectionLevel: "M", margin: 1, width: 240, color: { dark: "#18332d", light: "#ffffff" } });
