...
import { canBeSickNoteDoctor, dateFromInput, defaultCertificateWording, nextCertificateNumber, sickNoteInputSchema } from "@/lib/sick-notes";

function errorMessage(error: unknown) {
  if (error instanceof Error && ["PATIENT", "DOCTOR", "APPOINTMENT"].includes(error.message)) {
    return error.message === "PATIENT" ? "Choose an active patient." : error.message === "DOCTOR" ? "Choose an active authorised doctor." : "The appointment does not belong to this patient.";
  }
...
export async function POST(request: Request) {
  ...
  if (!session) return NextResponse.json({ error: "Only an authorised owner, administrator, or doctor can create sick notes." }, { status: 403 });
  ...
  try {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        const note = await db.$transaction(async (tx) => {
          ...
          const certificateNumber = await nextCertificateNumber(tx, new Date().getFullYear());
          const certificateWording = input.certificateWording || defaultCertificateWording(input);
          const created = await tx.sickNote.create({ data: {
            certificateNumber, patientId: patient.id, appointmentId: appointment?.id || null, doctorUserId: doctor.id,
            purpose: input.purpose, consultationDate: dateFromInput(input.consultationDate), consultationTime: input.consultationTime || null,
            ...
            doctorNotes: input.doctorNotes, certificateWording, aiDraftUsed: input.aiDraftUsed,
            createdById: session.id, updatedById: session.id,
          } });
          await tx.activityLog.create({ data: { userId: session.id, action: "SICK_NOTE_CREATED", entityType: "SickNote", entityId: created.id, summary: `Draft ${created.certificateNumber} created` } });
          return created;
        }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
        return NextResponse.json({ id: note.id, certificateNumber: note.certificateNumber, status: note.status }, { status: 201 });
      } catch (error) {
    ...
    return NextResponse.json({ error: errorMessage(error) }, { status: error instanceof Error && ["PATIENT", "DOCTOR", "APPOINTMENT"].includes(error.message) ? 400 : 500 });
  }
}
