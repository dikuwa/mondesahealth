...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  return NextResponse.json({ name: invitation.name, email: invitation.email, practice: invitation.practice.name });
}
...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    await db.$transaction(async (tx) => {
      ...
      await tx.practiceUser.create({ data: { practiceId: invitation.practiceId, userId: user.id, role: invitation.role, permissions: user.permissions } });
