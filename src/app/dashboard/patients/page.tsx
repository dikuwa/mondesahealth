...
export default async function Patients(){
  ...
  const[patients,funds,session]=await Promise.all([
    db.patient.findMany({
      ...
      include:{
        memberships:{where:{practiceId:authSession.practiceId,current:true},include:{medicalAid:true}},
        _count:{select:{appointments:{where:{practiceId:authSession.practiceId}},claims:{where:{practiceId:authSession.practiceId}}}},
      },
