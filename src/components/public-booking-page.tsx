...
export default async function PublicBookingPage({ practiceId }: { practiceId?: string }) {
  const settingsPracticeId = practiceId || ORIGINAL_PRACTICE_ID;
  const [funds, settings, emergencyRows, departmentPractices] =
    await Promise.all([
      ...
      db.practiceSetting.findUnique({
        where: { practiceId: settingsPracticeId },
      }),
      db.emergencyContact.findMany({
        where: { practiceId: settingsPracticeId, active: true },
        orderBy: [{ primary: "desc" }, { sortOrder: "asc" }],
      }),
      db.practice.findMany({
        where: { status: "ACTIVE", publicVisible: true, ...(practiceId ? { id: practiceId } : {}) },
        select: {
          id: true,
          name: true,
          services: {
          ...
          setting: { select: { bookingMode: true, aiIntakeEnabled: true, aiImageEnabled: true } },
          emergencyContacts: { where: { active: true }, orderBy: [{ primary: "desc" }, { sortOrder: "asc" }] },
        },
        orderBy: { name: "asc" },
      }),
    ]);
  ...
  const departments = departmentPractices.flatMap((practice) => {
    ...
    return ids.map((id) => {
      ...
      return {
        ...
        mode: practice.setting?.bookingMode || "AVAILABLE_TIME",
        aiIntakeEnabled: practice.setting?.aiIntakeEnabled || false,
        aiImageEnabled: practice.setting?.aiImageEnabled || false,
        emergencyContacts: orderEmergencyContacts(practice.emergencyContacts),
