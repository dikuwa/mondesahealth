...
export function SubscriptionManager({
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget),
      toastId = toast.loading("Creating plan…");
    try {
    ...
    <>
      <div className="manager-toolbar">
        <div>
          ...
          <p>
            Plans restrict selected new actions only; clinical data is never
            deleted for billing status.
          </p>
        </div>
            ...
            <tbody>
              {plans.map((x) => (
                <tr key={x.id}>
                  <td>
                    <b>{x.name}</b>
                    <small style={{ display: "block" }}>{x.description}</small>
                  </td>
                  <td>{x.billingFrequency}</td>
                  <td>N${x.fee.toFixed(2)}</td>
                  <td>{x.gracePeriodDays} days</td>
            ...
            <div className="form-grid">
              ...
              <label>
                ...
                <CustomSelect
                  ...
                  options={["MONTHLY", "QUARTERLY", "ANNUAL", "FIXED"].map(
