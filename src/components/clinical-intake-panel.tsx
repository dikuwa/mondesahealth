...
import { Bot, Check, ExternalLink, Loader2, Save, X } from "lucide-react";
...
export function ClinicalIntakePanel({ intake, canUseAi, onUpdated }: { intake: ClinicalIntake; canUseAi: boolean; onUpdated: () => void }) {
  ...
  return <section className="clinical-intake-card">
    <div className="clinical-intake-heading"><div><span className="eyebrow">Patient symptom intake</span><h3>Patient-reported information</h3></div><span className="status-badge">{intake.reviewStatus.replaceAll("_", " ")}</span></div>
    <div className="notice-warning">AI-organised content is not clinically verified. Independently verify every statement and suggestion.</div>
    <div className="clinical-intake-copy"><b>Original reason for visit</b><p>{intake.originalReason}</p></div>
    {intake.approvedSummary && <div className="clinical-intake-copy"><b>AI-organised summary · Patient approved</b><p>{intake.approvedSummary}</p></div>}
    {intake.clinicianCorrections && <div className="clinical-intake-copy is-reviewed"><b>Edited by clinician</b><p>{intake.clinicianCorrections}</p></div>}
    <dl className="clinical-intake-fields">{Object.entries(intake.fields).filter(([, value]) => value !== null && value !== "").map(([key, value]) => <div key={key}><dt>{fieldLabels[key] || key}</dt><dd>{String(value)}{key === "severity" ? "/10" : ""}</dd></div>)}</dl>
    ...
    <details className="clinical-intake-conversation"><summary>Conversation and consent details</summary><p>AI consent: {intake.aiConsent ? "Captured" : "Not captured"} · Image consent: {intake.imageConsent ? "Captured" : "Not captured"}</p><p>Consent: {intake.consentAt ? new Date(intake.consentAt).toLocaleString("en-NA") : "Not captured"} · Summary generated: {intake.summaryGeneratedAt ? new Date(intake.summaryGeneratedAt).toLocaleString("en-NA") : "Not generated"} · Patient approved: {intake.patientApprovedAt ? new Date(intake.patientApprovedAt).toLocaleString("en-NA") : "Not approved"} · Clinician reviewed: {intake.clinicianReviewedAt ? new Date(intake.clinicianReviewedAt).toLocaleString("en-NA") : "Not reviewed"}</p>{intake.messages.map((message, index) => <p key={index}><b>{message.role === "PATIENT" ? "Patient" : "AI assistant"}:</b> {message.skipped ? "Question skipped" : message.content}</p>)}{intake.questionsSkipped.length > 0 && <p><b>Skipped:</b> {intake.questionsSkipped.join("; ")}</p>}</details>
    {editing ? <div className="clinical-summary-edit"><label className="field"><span>Clinician corrections</span><textarea className="input" value={corrections} onChange={(event) => setCorrections(event.target.value)} maxLength={2000}/></label><div className="patient-ai-actions"><button className="btn btn-light" type="button" onClick={() => setEditing(false)}><X size={15}/> Cancel</button><button className="btn btn-primary" disabled={loading || corrections.trim().length < 10} onClick={() => update({ action: "EDIT_SUMMARY", summary: corrections }, "Clinician corrections saved")}><Save size={15}/> Save correction</button></div></div> : <div className="patient-ai-actions"><button className="btn btn-light" type="button" onClick={() => setEditing(true)}>Correct summary</button><button className="btn btn-light" type="button" disabled={loading || intake.reviewStatus === "REVIEWED"} onClick={() => update({ action: "MARK_REVIEWED" }, "Intake marked reviewed")}><Check size={15}/> Mark reviewed</button>{canUseAi && <button className="btn btn-light" type="button" onClick={() => setAssistantOpen((value) => !value)}><Bot size={16}/> Clinical AI assistant</button>}</div>}
