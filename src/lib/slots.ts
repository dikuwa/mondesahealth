...
export async function availableSlots(dateString: string, now = new Date(), practiceId = "mondesa-health", providerId?: string, serviceId?: string) {
  const date = new Date(`${dateString}T00:00:00`);
  if (Number.isNaN(date.getTime())) return [];
  const setting = await db.practiceSetting.findUnique({ where: { practiceId } });
  ...
  const rule = await db.availabilityRule.findUnique({ where: { practiceId_weekday: { practiceId, weekday: date.getDay() } } });
  if (!rule?.active) return [];
  const dayStart = startOfDay(date);
  const nextDay = addMinutes(dayStart, 1440);
  const [appointments, blocks, holds, providerRule, service] = await Promise.all([
    db.appointment.findMany({ where: { practiceId, ...(providerId ? { providerId } : {}), startAt: { gte: dayStart, lt: nextDay }, status: { notIn: ["CANCELLED", "NO_SHOW"] } } }),
    db.blockedTime.findMany({ where: { practiceId, ...(providerId ? { OR: [{ providerId: null }, { providerId }] } : { providerId: null }), startAt: { lt: nextDay }, endAt: { gt: dayStart } } }),
    db.appointmentChangeRequest.findMany({where:{appointment:{practiceId},status:"PENDING",expiresAt:{gt:now},proposedStartAt:{gte:dayStart,lt:nextDay}}}),
    providerId ? db.providerAvailability.findUnique({ where: { providerId_weekday: { providerId, weekday: date.getDay() } } }) : null,
    serviceId ? db.departmentService.findFirst({ where: { id: serviceId, practiceId, active: true } }) : null,
  ]);
  ...
  const lunchStart = providerRule?.breakStart ? parse(providerRule.breakStart, "HH:mm", date) : rule.lunchStart ? parse(rule.lunchStart, "HH:mm", date) : null;
  const lunchEnd = providerRule?.breakEnd ? parse(providerRule.breakEnd, "HH:mm", date) : rule.lunchEnd ? parse(rule.lunchEnd, "HH:mm", date) : null;
...
export async function nextAvailableSlot(now = new Date(), practiceId = "mondesa-health") {
  const setting = await db.practiceSetting.findUnique({ where: { practiceId } });
