...
const styles = StyleSheet.create({ page: { fontFamily: "Onest", fontSize: 10, padding: 42, color: "#18332d" }, header: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", borderBottomWidth: 1, borderBottomColor: "#cbd7d1", paddingBottom: 15 }, headerDetails: { maxWidth: 290, color: "#60736d", fontSize: 8, lineHeight: 1.25, textAlign: "right" }, title: { fontFamily: "Inter Tight", fontSize: 25, fontWeight: 700, marginTop: 22 }, block: { marginTop: 18, padding: 14, backgroundColor: "#f3f6f4" }, row: { flexDirection: "row", justifyContent: "space-between", marginTop: 8 }, footer: { position: "absolute", bottom: 35, left: 42, right: 42, borderTopWidth: 1, borderTopColor: "#dce4df", paddingTop: 9, color: "#647871" } });
...
export function ReceiptDocument({ receipt, practice }: { receipt: ReceiptData; practice: PracticeSetting }) {
  ...
  return <Document title={`Receipt ${receipt.number}`} author={practice.practiceName}><Page size="A4" style={styles.page}>
    <View style={styles.header}><DocumentBrand /><Text style={styles.headerDetails}>{practice.practiceName}{"\n"}{practice.doctorName}{"\n"}Practice no: {practice.practiceNumber} · Registration no: {practice.registrationNumber}{"\n"}{practice.address}{"\n"}{practice.phone} · {practice.email}</Text></View>
    ...
    <View style={{ marginTop: 34 }}><DocumentSignature name={practice.signatureName} title={practice.signatureTitle} /></View>
    <View style={styles.footer}><Text>{practice.practiceName} · {practice.address} · {practice.phone}</Text><Text style={{ marginTop: 4 }}>This receipt confirms a payment recorded by the practice management system.</Text></View>
  </Page></Document>;
}
