...
export async function getDueReminders(practiceId: string, now = new Date()) {
  ...
  const appointments = await db.appointment.findMany({
    where: {
      ...
      startAt: {
        gt: now,
        lte: addHours(now, setting?.reminderLeadHours ?? 24),
      },
    },
  ...
  return appointments.flatMap((appointment) => {
    const reminder = appointment.reminders.find(
      (item) =>
        item.appointmentStartAt.getTime() === appointment.startAt!.getTime(),
    );
    ...
    return [
      {
        ...
        status: reminder?.status || "DUE",
        reference: appointment.reference,
        patient: appointment.patient.fullName,
        phone: appointment.patient.phone,
        whatsapp: appointment.patient.whatsapp,
